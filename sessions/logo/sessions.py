...
def fetch_events_from_postgres(start_date_str, end_date_str, db_config):
    ...
    tz_name = os.environ.get("TZ_NAME", "Asia/Yerevan")
    # Parse input dates
    start_dt_local = pd.Timestamp(f"{start_date_str} 02:00:00").tz_localize(tz_name)
    end_dt_local = pd.Timestamp(f"{end_date_str} 01:59:59") + pd.Timedelta(days=1)
    end_dt_local = end_dt_local.tz_localize(tz_name)
...
def convert_timestamp(df):
    ...
    print(f"Converting '{ts_col}' to {os.environ.get('TZ_NAME', 'Asia/Yerevan')}")

    datetime_utc = pd.to_datetime(df[ts_col], unit='s', utc=True)
    datetime_local = datetime_utc.dt.tz_convert(os.environ.get("TZ_NAME", "Asia/Yerevan"))
...
class MemberViewershipProcessor:
 ...
 # Return as-is if not found
                ...
                output_filename = "logo_sessions.csv"
...
if __name__ == "__main__":
    ...
    tz = pytz.timezone(os.environ.get("TZ_NAME", "Asia/Yerevan"))
    ...
    OUTPUT_DIR = BASE_DIR / 'sessions' / 'logo' / 'household_viewership_memberwise_output'
